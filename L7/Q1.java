...
class Polygon{
	...
	boolean isInside(Node point){
		if (n == 1){
			...
			int node1 = this.points.length;
			int i = 0, count = 0;
			do{
				int next = (i+1)%node1;
				if (points[i].x == points[next].x){
					if (point.x == points[i].x){
						if (point.y >= Math.min(points[i].y,points[next].y) && 
							point.y <= Math.max(points[next].y,points[i].y)){
					...
					if (point.y == points[i].y){
						if (point.x >= Math.min(points[i].x,points[next].x) && 
							point.x <= Math.max(points[next].x,points[i].x)){
							return true; 
						}
						else{
							return false;
...
class KDTree{
	...
	private void insertUtil(Node node1, Node node2){
		if (node1.isXaligned){
			if (node2.x >= node1.x){
				if (node1.right == null){
					node1.right = node2;
					node2.isXaligned = false;
				}
				else{
					insertUtil(node1.right,node2);
				}
			}
			else{
				if (node1.left == null){
